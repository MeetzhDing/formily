...
const { provide: provideAccount, consume: consumeAccount } = createFormContextFactory<IAccount>('account');

type IWhitelist = Record<string, boolean>;
const { provide: provideWhitelist, consume: consumeWhitelist } = createFormContextFactory<IWhitelist>('whitelist');
